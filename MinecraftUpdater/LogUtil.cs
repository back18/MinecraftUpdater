...
namespace MinecraftUpdater
{
    public static class LogUtil
    {
        static LogUtil()
        {
            ...
            XmlConfigurator.Configure(new FileInfo(SR.MinecraftUpdaterDirectory.LogConfigFile));
            ...
            _file.File = SR.MinecraftUpdaterDirectory.LogsDir.LatestFile;
        ...
        {
            FileInfo logFileInfo = new(SR.MinecraftUpdaterDirectory.LogsDir.LatestFile);
            if (!logFileInfo.Exists)
                return;

            byte[] logBytes = File.ReadAllBytes(SR.MinecraftUpdaterDirectory.LogsDir.LatestFile);
            string format = SR.MinecraftUpdaterDirectory.LogsDir.Combine(logFileInfo.LastWriteTime.ToString("yyyy-MM-dd") + "-{0}.log.gz");
